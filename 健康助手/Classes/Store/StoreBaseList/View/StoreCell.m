...
{
    [btn setSelected:YES];
    [StoreTool DetailWithParam:@{@"id":@(self.storeModel.ID)} success:^(id mesicines) {
        Store* store = mesicines;
        [StoreTool  saveStore:store];
    } failure:^(NSError *err) {
        [btn setSelected:NO];
    }];
...
-(void)setStoreModel:(Store *)storeModel
{
    _storeModel = storeModel;
    // 1.配图

    [bHttpTool downloadImage:[imageBaseURL stringByAppendingPathComponent:_storeModel.image] place:[UIImage imageNamed:@"timeline_image_loading.png"] imageView:self.imageView];
    
    
    if ([StoreTool existCorrentStore:_storeModel.ID]) {
    ...
    {
        //名称
        self.name.text = _storeModel.name;
        //类型
        
        self.leftTitle.text = _storeModel.address;
        //科室
        self.rightTitle.text = _storeModel.type ;
    }else{
        self.name.text = _storeModel.title;
        
        self.leftTitle.text = [NSString stringWithFormat:@"搜索次数:%d",_storeModel.scanTimes] ;
        
        self.rightTitle.text = _storeModel.content ;

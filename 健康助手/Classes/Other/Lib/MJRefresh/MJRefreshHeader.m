...
//        // 初始化文字
//        [self setTitle:NSLocalizedString(@"PullDownToRefresh",nil) forState:MJRefreshHeaderStateIdle];
//        [self setTitle:NSLocalizedString(@"ReleaseToRefreshData", nil) forState:MJRefreshHeaderStatePulling];
//        [self setTitle:NSLocalizedString(@"PleaseWaitWhileLoading", nil) forState:MJRefreshHeaderStateRefreshing];
        // 初始化文字
        [self setTitle:@"下拉刷新" forState:MJRefreshHeaderStateIdle];
        [self setTitle:@"释放更新" forState:MJRefreshHeaderStatePulling];
        [self setTitle:@"正在刷新，请等待！！！" forState:MJRefreshHeaderStateRefreshing];
    }
    return self;
}
...
//        // 3.显示日期
//
//        self.updatedTimeLabel.text = [NSString stringWithFormat:@"%@：%@ %@",        NSLocalizedString(@"RecentUpdates", nil), NSLocalizedString(@"Today", nil),time];
//    } else {
//        self.updatedTimeLabel.text = [NSString stringWithFormat:@"%@：",        NSLocalizedString(@"RecentUpdates", nil)];
        
        // 3.显示日期
        
        self.updatedTimeLabel.text = [NSString stringWithFormat:@"%@：%@ %@",        @"最近刷新时间", NSLocalizedString(@"Today", nil),time];
    } else {
        self.updatedTimeLabel.text = [NSString stringWithFormat:@"%@：",        @"最近刷新时间"];
    }
}
